package com.yffsc.library.transition;
...
import com.yffsc.library.anim.SceneFade;
import com.yffsc.library.anim.SceneScaleUp;
import com.yffsc.library.anim.ViewAnim;
import com.yffsc.library.util.Position;
...
public class TransitionCompat {
    ...
    private static int mLayoutResId;//返回动画view的id
    // 分享元素的id和id list
    private static int mSharedElementId;//返回动画view的id
    private static ArrayList<Integer> mSharedElementIds;//返回动画view的id
    // 分享元素的绘制区域
    ...
    public static void setEndString(String endString) {
        TransitionCompat.endString = endString;
    }

    public static void setStartString(String startString) {
        TransitionCompat.startString = startString;
    }
     ...
     * @param isStart：是否是开始动画，如果是fale则执行结束的动画           执行ScaleUpAnimation动画，包括开始和结束的动画效果。
     *                                                  【开始】
    ...
    /**
     * @param activity：当前的activity，可以是actionbarActivity
     * @param isStart：是否是开始动画，如果是fale则执行结束的动画           为什么api里面有这个方法呢，其实很简单要想实现一个view放到渐变为activity的效果就必须让另一个
     ...
     * @param isStart:是否是开始动画，如果是fale则执行结束的动画       执行SceneTransitionAnimation动画效果，包括开始和结束。（在view显示完整后才能有效）
     *                                              【开始】
    ...
    private static View getSharedElementOrginalView(Activity activity, int layoutResId) {
        ...
        ((TextView)orginalView).setText(endString);
        if (orginalView == null) {
